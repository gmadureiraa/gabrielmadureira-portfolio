...
const features = [
        ...
        >
          <div className="absolute top-2 right-2 bg-background rounded-lg px-4 py-2 text-xs text-neutral-500 dark:text-neutral-300 max-w-3/4 w-fit">
            <div className="flex items-center gap-2">
              <div
                className={`w-3 h-3 rounded-full animate-pulse ${process.env.NEXT_PUBLIC_AVAILABLE_FOR_FREELANCE == "true"
                    ? "bg-emerald-400"
                    : "bg-yellow-400"
                  }`}
              ></div>
...

  {
    ...
    background: (
      ...
      >
        ...
        >
          {[
            { name: "Kaleidos Digital", body: "Agência de marketing cripto. R$602k em 2025.", href: "/projects/kaleidos-digital", type: "Projeto" },
            { name: "Context Engineering", body: "Como gerencio 5 clientes com 1 pessoa e IA.", href: "/posts/context-engineering-gestao-5-clientes", type: "Artigo" },
            { name: "KAI APP", body: "SaaS de gestão de marketing com IA.", href: "/projects/kaleidos-digital", type: "Projeto" },
            { name: "Meta Ads 100%", body: "O que muda quando a IA cria seus anúncios.", href: "/posts/meta-ads-100-automacao-o-que-isso-significa", type: "Artigo" },
            { name: "GOS", body: "Sistema operacional pessoal com Claude Code.", href: "/projects/automacoes-marketing-n8n", type: "Projeto" },
            { name: "R$602k e uma crise", body: "O melhor ano financeiro e o modelo que não escala.", href: "/posts/kaleidos-r602k-o-que-aprendi", type: "Artigo" },
            { name: "Jornal Cripto", body: "Portal de análise cripto. 10k visitas/mês no pico.", href: "/projects/jornal-cripto", type: "Projeto" },
          ].map((item, idx) => (
            <a
              href={item.href}
              key={idx}
              className={cn(
                "relative w-44 h-full cursor-pointer overflow-hidden rounded-xl border p-4 hover:-translate-y-1",
...

  {
    Icon: "",
    name: "Newsletter",
    description: "Inscreva-se para receber minhas melhores ideias, táticas de marketing e growth hacking.",
    className: "col-span-3 md:col-span-1",
    href: "/newsletter",
    ...
    background: (
      <div className="absolute h-full w-full left-0 top-0 origin-top rounded-md transition-all duration-300 ease-out group-hover:scale-[105%]">
        <div className="absolute h-full w-full [mask-image:linear-gradient(to_top,transparent_40%,#000_90%)]">
          <RippleCard />
        </div>
      </div>
    ),
